...
def export_route_gpx(route_id: int) -> dict:
    """
    ...
    try:
        ...
        # Fetch GPX data from Strava API
        ...
        return {
            "content": [{"type": "text", "text": f"✅ Route {route_id} exported successfully as GPX to: {full_path}"}],
        }

    except requests.RequestException as e:
        error_message = str(e)
        print(f"Error in export_route_gpx tool for route {route_id}: {error_message}")
        return {
            "content": [{"type": "text", "text": f"❌ Error exporting route {route_id} as GPX: {error_message}"}],
            "isError": True
        }
